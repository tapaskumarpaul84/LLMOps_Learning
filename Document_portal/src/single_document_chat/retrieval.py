...
class ConversationalRAG:
    ...
    def _get_session_history(self,session_id: str) -> BaseChatMessageHistory:
        try:
            ...
            self.log.error("Failed to access session history", session_id=self.session_id, error=str(e))
    ...
    def load_retriever_from_faiss(self,index_path:str):
        try:
            ...
            if not os.path.isdir(index_path):
                raise self.log.info(f"FAISS index directory not found. {index_path}")
            
            vectorestore=FAISS.load_local(index_path,embeddings=embeddings)
