...
class SingleDocIngestor:
    def __init__(self,data_dir:str='data/single_document_chat',faiss_dir:str='faiss_index'):
        try:
            ...
            #self.faiss_dir=os.makedirs(faiss_dir,exist_ok=True)


            self.model_loader=ModelLoader()
            self.log.info("SingleDocumentIngestor initialized",temp_path=str(self.data_dir),faiss_path=str(self.faiss_dir))
    ...
    def _create_retriever(self,documents):
        try:
            spliter=RecursiveCharacterTextSplitter(chunk_size=1000,chunk_overlap=300)
            chunks=spliter.split_documents(documents)
            self.log.info("Documents splited into chunks",count=len(chunks))
            print("#######################chunks count############")
            print(len(chunks))
            print("################chunk added#################")
            ...
            self.log.info("FAISS index vectorstore created",faiss_path=str(self.faiss_dir))
