...
class DocumentIngestor:
    ...
    def __init__(self,temp_dir:str="data/multi_doc_chat",faiss_dir:str="faiss_index",session_id: str | None=None):
        try:
            ...
            self.log.info("Document Ingestor initialized",temp_base=str(self.temp_dir),faiss_base=str(self.faiss_dir),
                          session_id=self.session_id,temp_path=str(self.session_temp_dir),
    ...
    def ingest_files(self,uploaded_files):
        try:
            ...
            for uploaded_file in uploaded_files:
                ...
                if not documents:
                    self.log.error("Failed to load documents",str(e))
                    raise DocumentPortalException("No valid document loaded.",sys)

            self.log.info("All files loaded successfully",total_docs=len(documents),session_id=self.session_id)
            return self._create_retriever(documents)


        except Exception as e:
            self.log.error("Failed to ingest files",error=str(e))
            raise DocumentPortalException("Files Ingestion error",sys)
    ...
    def _create_retriever(self,documents):
        try:
            ...
            self.log.info("Documents splited into chunks",total_chunk=len(chunks),session_id=self.session_id)
            ...
            self.log.info("FAISS index saved to the disk",path=str(self.session_faiss_dir),session_id=self.session_id)
